...
from flask_security import SQLAlchemyUserDatastore, Security, hash_password, SQLAlchemySessionUserDatastore
...
import config
from ceredira_tess.database import db_session, init_db
...
def create_app():
    ...
    with app.app_context():
        ...
        admin.add_view(UserModelView(User, db_session, endpoint='User', name='Пользователи'))
        admin.add_view(RoleModelView(Role, db_session, endpoint='Role', name='Роли'))
        admin.add_view(OperationSystemTypeModelView(OperationSystemType, db_session, endpoint='OperationSystemType',
                                                    name='Типы ОС'))
        admin.add_view(AgentModelView(Agent, db_session, endpoint='Agent', name='Агенты'))
        admin.add_view(ScriptModelView(Script, db_session, endpoint='Script', name='Скрипты'))
